            ...
            methods: {
                ...
                loadPicCode: function loadPicCode() {
                    ...
                    this.$http.get(this.baseUrl + 'CodeOnle?imgKey=' + this.timestamp, { 'imgKey': this.timestamp }).then(function (res) {
                        $('.picCode').attr('src', _this7.baseUrl + 'CodeOnle?imgKey=' + _this7.timestamp);
                        console.log(_this7.timeStamp);
                    });
                },
                //获取验证码后样式改变
                        ...
                        switch (res.data.rcd) {
                            case 'R0001':
                                if (window.location.href.indexOf('sn=5227922') != -1) {
                                    monitor.request({ type: "post", url: monitor.data_url, data: monitor.url_parameter, success: function success() {
